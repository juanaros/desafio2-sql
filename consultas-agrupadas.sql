...
SELECT fuente, SUM(cantidad) AS "total_inscritos" FROM inscritos 
GROUP BY fuente 
...
FROM (SELECT fecha, SUM(cantidad) AS "total_inscritos" FROM inscritos GROUP BY fecha) AS "t";
...
SELECT fecha, SUM(cantidad) AS "cantidad_total", ROUND(AVG(cantidad), 2) AS "promedio" FROM inscritos 
GROUP BY fecha 
